...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // رفع صورة جديدة إذا تم اختيار صورة
    $image = $product['image']; // الاحتفاظ بالصورة القديمة
    $newImage = null;
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare(
            "UPDATE products
             SET category_id = ?, image = ?, name = ?, description = ?, price = ?, quantity = ?
             WHERE id = ?"
        );
...
<div class="form-wrapper mx-auto">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body p-4 p-md-5">
            <div class="mb-4">
                <p class="text-primary fw-semibold mb-1">PRODUCT #<?= (int) $id ?></p>
                <h1 class="h3 fw-bold">Edit product</h1>
                <p class="text-secondary mb-0">Update the product information.</p>
            </div>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <textarea id="description" name="description" class="form-control"
                              rows="4" maxlength="2000"><?= e($data['description']) ?></textarea>
                ...
                <div class="mb-3">
                    ...
                    <div class="form-text">Leave empty to keep the current image. Maximum size: 2 MB.</div>
                    <?php if (!empty($product['image'])): ?>
                        <img id="imagePreview" class="image-preview mt-3"
                             src="<?= e(baseUrl('uploads/products/' . rawurlencode($product['image']))) ?>"
